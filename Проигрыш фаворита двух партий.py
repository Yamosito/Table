...
#PROXY = '103.216.51.210:8191' # только HTTPS прокси
    ...
    try:
        ...
        for match in matches:
            try:
                ...
                coef1 = match.find('div', 'c-bets').find_all('a')[0].get_text().strip()
                coef2 = match.find('div', 'c-bets').find_all('a')[2].get_text().strip()
                link = match.find('a', 'c-events__name').get('href')
                link = 'https://1xstavka.ru/' + link
                print(name, score1, score2, coef1, coef2)

                result = write_file(link, 'prematch_coefs2') #запись в файл с пре-кэфами 
                text = link, coef1, coef2
                if(result):
                    with open('prematch_coefs2.txt', 'a', encoding='utf-8') as f:
                        f.writelines(str(text) +"\n")

                if(score1 == '0') and ((score2 == '1') or (score2 == '2')):
                    with open('prematch_coefs2.txt', 'r', encoding='utf-8') as f:
                        for line in f:
                            if(link in line):
                                ...
                                if(coef1 != '-') and (coef2 != '-')  and (prematch_coef1 != '-') :
                                    if(float(coef1) > 3.5) and (1.1 <= float(prematch_coef1) <= 1.35):
                                        result = write_file2(link, 'signals2')
                                        if(result):
                                            send_message(chatID, 'Алгоритм 1.2\n' + str(name) +  '\nКФ до матча: ' + str(prematch_coef1) + ' - ' + str(prematch_coef2) + '\nLive КФ: ' + str(coef1) + ' - ' + str(coef2) + '\nРекомендация:' + ' Победа в 3 партии ', link)
                        ...
                        for line in f:
                            if(link in line):
                                ...
                                if(coef1 != '-') and (coef2 != '-') and (prematch_coef2 != '-'):
                                    if(float(coef2) > 3.5) and (1.1 <= float(prematch_coef2) <= 1.35):
                                        result = write_file2(link, 'signals2')
                                        if(result):
                                            send_message(chatID, 'Алгоритм 1.2\n' + str(name) +  '\nКФ до матча: ' + str(prematch_coef1) + ' - ' + str(prematch_coef2) + '\nLive КФ: ' + str(coef1) + ' - ' + str(coef2) + '\nРекомендация:' + ' Победа в 3 партии ', link)
